...
class mits_userlike
{
    ...
    function __construct()
    {
        ...
        if (defined('MODULE_COOKIE_CONSENT_STATUS') && MODULE_COOKIE_CONSENT_STATUS == 'true' && defined($this->name . '_ORGANISATIONS_ID') && constant(
            $this->name . '_ORGANISATIONS_ID'
          ) != '' && defined($this->name . '_COOKIE_CONSENT_PURPOSEID') && constant($this->name . '_COOKIE_CONSENT_PURPOSEID') != '') {
            if (xtc_db_num_rows(xtc_db_query("SHOW TABLES LIKE '" . TABLE_COOKIE_CONSENT_COOKIES . "'"))) {
                xtc_db_query(
                  "UPDATE " . TABLE_COOKIE_CONSENT_COOKIES . " SET cookies_list = REPLACE(cookies_list, '{organization-id}', '" . constant(
                    $this->name . "_ORGANISATIONS_ID"
                  ) . "'), cookies_description = REPLACE(cookies_description, '{organization-id}', '" . constant($this->name . "_ORGANISATIONS_ID") . "') WHERE cookies_id = " . constant(
...
'
        ...
        xtc_db_query(
          "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . $this->name . "_WIDGET_CODE', '', 6, 2, NULL, now());"
        );
        xtc_db_query(
          "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . $this->name . "_ORGANISATIONS_ID', '', 6, 3, NULL, now());"
        );
        xtc_db_query(
          "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . $this->name . "_IN_COOKIE_CONSENT', '" . $userlike_in_cookie_consent . "', 6, 4, 'xtc_cfg_select_option(array(\'true\', \'false\'), ', now())"
        );
        xtc_db_query(
          "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . $this->name . "_COOKIE_CONSENT_PURPOSEID', '" . $purpose_id . "', 6, 5, NULL, now());"
        );
        xtc_db_query(
          "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . $this->name . "_VERSION', '" . $this->version . "', 6, 99, NULL, now())"
        );
